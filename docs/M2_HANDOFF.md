...
最終更新: 2026-04-24（`core::history` 本体 landed、次は `core::rename`）
...
- 次: `core::rename`。preview → apply（原子トランザクション）、`core::naming` の NameCandidate を入力に取り `FillMode` で穴を解消、history への append + undo/redo 連携、`progest clean --apply` / CLI `rename` の繋ぎ込み。
...
## 5. 次モジュール（`core::rename`）kickoff 質問テンプレ

`core::rename` 着手時に AskUserQuestion で確認する想定:

1. preview モデル: `Vec<RenameOp>` に conflicts / suggested-from を埋める形で良いか。`RenameOp` を pub にして CLI/UI が直接触る前提か、serde 越しの wire type を噛ませるか
2. apply の原子性実装: shadow copy + temp dir（十字結び回避）/ `renameat2(RENAME_EXCHANGE)` / `rename` 2-phase で rollback。macOS/Linux の差をどこで吸収するか
3. watch quiet window: apply 中の rename イベントを `core::watch` が拾わない工夫（静音フラグ / 応答期間抑制 / ignore-by-file-id）。今回は rename 側がフラグを立てて watch が見に行く案で良いか
4. `core::history` 連携: `history::Store` を `Rename::apply` が `&dyn Store` で受け取り、成功時のみ `append(Operation::Rename { ... })`。bulk rename は `group_id` を生成して束ねる方針で良いか
5. `progest clean --apply` / `progest rename --apply` の CLI 配線: `core::rename` の内部 API を両方が同じ entry point で叩く（薄い CLI adapter）で良いか、rename 側が Subject としてファイル集合を切る責務も持つか
