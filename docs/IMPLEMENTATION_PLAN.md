...
最終更新: 2026-04-27（M3 polish ラウンド完了 + frontend toolchain を Vite+ に統合: oxlint + oxfmt + tsgo を `mise run check` / lefthook pre-commit に組込）
...
- **M3 検索とビュー**: 進行中。`core::search` + FTS5 + コマンドパレット UI + tree/flat view + ディレクトリインスペクター + `is:misplaced` + views.toml + CLI `search`/`tag` + Tauri IPC 同時実装。詳細は §5 M3、進行中の引き継ぎは [`docs/M3_HANDOFF.md`](./M3_HANDOFF.md)。
  ...
  - [ ] M3 #9 ディレクトリインスペクター + placement バッジ + `accepts.{read,write}` IPC（M3 完了の最終 piece）
