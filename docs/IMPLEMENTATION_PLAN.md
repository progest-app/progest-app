...
- **M3 検索とビュー**: 完了 — `core::search`（parser + AST + validate + planner + executor、102 test）+ FTS5 trigram + コマンドパレット UI + tree/flat view（list/grid + DataTable）+ ディレクトリインスペクター（accepts 編集）+ ファイルインスペクター（tag/notes 編集）+ project init UI + `is:misplaced` + views.toml + CLI `search`/`tag`/`view` + Tauri IPC 全実装 + polish（theme toggle / DSL コンマリスト + `::alias` / `>command` パレット / StatusBar / Vite+ 統合）+ `@vitejs/plugin-react` v6 oxc 移行。完了アーカイブは [`docs/M3_HANDOFF.md`](./M3_HANDOFF.md)。
  ...
  - [x] tree view + flat view（list / grid 切替）+ saved-views UI + `view.{list,save,delete}` / `files_list_dir` IPC — `core::search::views::View` に `display: ViewDisplay { List, Grid }` 追加（serde default `list`、forward-compat: 既存 views.toml は欠落キーが list にフォールバック）、`progest view save --display list|grid` CLI 拡張、`progest-tauri` に `view_list/save/delete` + tree view 用 `files_list_dir(path)`（`std::fs::read_dir` + `IgnoreRules` + dotfile 隠し + `..` エスケープガード、ファイルは `Index::get_file_by_path` + `rich_rows` で tags / violations / custom_fields 補完、未 reconcile も基本表示は出す）。`app/` は `<ProjectProvider>` で project / recent state を集約、メイン shell は `<TopBar />` + `<TreeView />`（lazy load + 展開キャッシュ）+ `<FlatView />`（200ms debounce 検索 + saved-view selector + list/grid toggle + Save-as ダイアログ + `[a-z0-9_-]{1,64}` id バリデーション）。`<CommandPalette />` は `useProject()` 化 + `onPickHit` 経由でメイン shell の詳細 Dialog にハンドオフ。`<ViolationBadges>` / `<ViolationDots>` を共有コンポーネントに切り出し（M3_HANDOFF §3.6 placeholder palette: naming amber / placement sky / sequence violet）。CLI smoke `view_save_persists_display_grid` + views golden 2 件追加（display round-trip + 欠落キーは List default）。`mise run check` + `pnpm build` green、`tauri dev` 実機 smoke は持ち越し（feat/m3-tree-flat-views）
  - [x] M3 polish ラウンド — UI / DSL の使い勝手を詰める追補 6 PR、いずれも shadcn skill 準拠で landing:
    - **theme toggle + 意味的バッジトークン**（feat/app-theme-toggle、PR #36）— `next-themes` で `progest:theme` localStorage、`<ThemeToggle>` (system/light/dark) を TopBar / Welcome に配置、`index.html` インラインスクリプトで FOUC 防止。`--badge-naming` / `--badge-placement` / `--badge-sequence` / `--warning` を `:root`+`.dark` に semantic token として定義し `@theme inline` で Tailwind 利用可能に、`text-amber-600 dark:text-amber-300` 系の手書き dark: override を `text-warning` / `bg-badge-naming/15` に置換（shadcn `rules/styling.md`「No manual `dark:` color overrides」準拠）。
    - **検索 DSL コンマリスト + `::alias` 展開**（feat/search-dsl-multi-and-alias、PR #37）— `type:png,jpg`/`tag:wip,review`/`scene:1,2,3` でコンマリスト OR 展開（quoted 値は literal で split 抑制）、`type::image` で `core::accepts` の AliasCatalog（builtin + `schema.toml [alias]`）を展開、`unknown_alias` / `unsupported_alias` / `empty_list_item` warning 追加。`validate_with_catalog(query, schema, aliases)` を新 entry point とし旧 `validate(query, schema)` は builtin alias 経由のラッパー、CLI / Tauri caller は project の AliasCatalog を load して渡す。validate 11 unit test 追加、`docs/SEARCH_DSL.md §4.4 / §4.10 / §4.11 / §9.2` 同時更新。
    - **空クエリ全件 + `.meta` / `.dirmeta.toml` 除外**（feat/empty-query-all-and-meta-filter、PR #38）— 新 IPC `files_list_all` で空クエリ時に index 全件を name-sorted で返す、`<FlatView>` 空入力 branch で fetch + 既存 list/grid renderer で表示。Tree view `build_dir_entry` に `is_sidecar_filename(name)` フィルタを追加、scanner `DEFAULT_PATTERNS` に `.dirmeta.toml` を追加（`<asset>.meta` は既存 `core::reconcile::is_sidecar` が処理）、`files_list_all` / `search_execute` の rich 結果を `is_plumbing_path` で post-filter（旧 build で indexed された stale 行のための defense-in-depth）。
    - **project 切替時の FlatView / TreeView リフレッシュ**（fix/flatview-refresh-on-project-change、PR #39）— 両コンポーネントが `useProject()` を subscribe、`project?.root` 変化時に query / response / error / activeView / display / cache / expanded を reset し新 root を再 fetch。検索 effect の deps に `project?.root` を追加して同一空クエリでも別プロジェクトで再実行。
    - **コマンドパレット `>command` モード + 拡張可能レジストリ**（feat/palette-system-commands、PR #40）— `app/src/lib/palette-commands/` に `PaletteCommand` interface + `fuzzyMatch` (AND-match)、`useProjectCommands` (Open project / Open recent: <name>) と `useThemeCommands` (Set theme: System/Light/Dark) を hook 単位で composition、`usePaletteCommands` aggregator に concat するだけで新カテゴリ追加可。`<CommandPalette>` は `query.startsWith(">")` で `<CommandsBody>` に切替、検索 effect スキップ、no-project でもアクセス可、長 hint は `min-w-0 max-w-[55%] truncate` + `title=`。新規ドキュメント [`docs/COMMAND_PALETTE.md`](./COMMAND_PALETTE.md) を canonical command list として追加（新コマンド追加時はそこに行を追記）。
    - **frontend toolchain を Vite+ に統合**（chore/m3-app-vite-plus）— `app/` の Vite 6 + tsc-only 体制を Vite+ に移行: `vite-plus` + `@voidzero-dev/vite-plus-core` を `app/` devDep に追加、`pnpm-workspace.yaml` overrides で transitive `vite` / `vitest` も Vite+ core/test に揃える。`app/vite.config.ts` の `defineConfig` を `vite-plus` から import し `fmt` / `lint` block を直書き、ESM 化のため `__dirname` → `import.meta.dirname`。lint ルールは `app/.oxlintrc.json`（categories=correctness/suspicious/perf、`src/components/ui/**` と `dist/**` を ignorePatterns）。`app/package.json` scripts は `dev/build/preview` を `vp` に切替、新規 `lint` / `fmt` / `fmt:check` / `check` (=`vp check && tsc --noEmit`)。`mise run check` の `pnpm --filter @progest/app typecheck` を `... check` に差替、`mise run fmt` に `pnpm --filter @progest/app fmt` を追加、lefthook pre-commit に `oxfmt` (glob `app/**/*.{ts,tsx,js,jsx,json,css,md,html}`) を追加（CI は `mise run check` を呼ぶので追加変更不要）。既存 `src/` には `vp fmt` を一括適用 + lint 0 warnings。
    - **ステータスバー + 入力クリア + Tooltip**（feat/status-bar、PR #41）— 画面下部に `<StatusBar>` を新設、左: 違反バッジ集約（naming/placement/sequence、ホバーで対象ファイル一覧を shadcn Tooltip 表示、retention 25 + `+N more`）、中央: project 名 + root（`min-w-0 truncate`）、右: active saved view chip（`<TooltipProvider>` を `App.tsx` 直下に）。`<FlatViewSummaryProvider>` で `<FlatView>` から `{ activeView, violationTotals, violationFiles }` を push、`<StatusBar>` は passive consumer。FlatView header の status row を残し（parse error / validate warnings / IPC error / hit count は input 直下を維持）、Input に末尾 `<X>` クリアボタン + Esc キーハンドラ追加（active view からの decouple 込み）。`<ViolationBadges>` を `titles` 受付に拡張、shadcn Tooltip primitive を `pnpm dlx shadcn@latest add tooltip` で追加。
  - [x] FlatView shadcn DataTable 移行（feat/flatview-data-table）— shadcn `table` primitive + `@tanstack/react-table ^8.21` を導入、`<HitsDataTable>`（columns: Path / Tags / Violations / Kind / Ext、controlled sorting + column visibility、custom sortingFns で Violations は合計件数 / Tags は join 文字列 / Ext は null safe）+ `<ColumnVisibilityMenu>`（shadcn `dropdown-menu` 追加、Path は disabled で常時可視）。FlatView 側に sort/columnVisibility state を lift（localStorage `progest:flatview-sorting` / `progest:flatview-columns` に永続）、`sortHits()` で table sortingFns を mirror して grid render path も同 sort を honors。toolbar は display mode で表示切替（list → ColumnVisibilityMenu、grid → `<GridSortControls>` = column Select + asc/desc Button）。`<HitList>` 撤去、`<HitGrid>` は sortedHits 受け。`views.toml` schema は不変（UI preference は view に保存しない方針）。pagination / row selection / column filter は意図的にスキップ。registry の `dropdown-menu` を `exactOptionalPropertyTypes: true` 下でビルド可能にするため `checked={checked ?? false}` に local fix。`mise run check` + `pnpm build` (549 KB / 168 KB gzip、tanstack で +76 KB) green。`tauri dev` 実機 smoke（列ヘッダソート / Columns dropdown / grid sort Select + 方向トグル / list↔grid 切替で sort 維持 / 永続化）は次セッションへ持ち越し
  - [x] file inspector + tag/notes 編集（feat/inspector-file-mode-tags-notes）— `progest-tauri::file_inspector_commands` 新設で 4 IPC: `tag_add(file_id, tag)` / `tag_remove(file_id, tag)`（`core::tag::{add, remove}` への薄ラッパー、validation + idempotent index write）/ `notes_read(path) -> { path, body, sidecar_exists }` / `notes_write(path, body)`（`core::meta::MetaStore` で原子的に sidecar を書き換え + `Index::set_search_projection` で FTS の notes 列を即時更新）。history 連携（tag_add / meta_edit op-kind）は CLI undo の "not yet wired" と歩調合わせて意図的に未実装。`<Shell>` の selection を `Selection = { kind: "dir"; path } | { kind: "file"; hit } | null` の排他 union に lift、TreeView dir click → dir、TreeView/FlatView/palette file click → file、`<InspectorPane>` が selection.kind で `<DirectoryInspector>` ↔ `<FileInspector>` を route。`<ResultDetailDialog>` は撤去（ファイル詳細は inspector に集約）、palette 内の `selected` フォールバック state も同時撤去。新規 `<FileInspector>` は path / name / kind / ext / file_id / violations / custom_fields の表示、タグ chip エディタ（Enter 追加 / Backspace-on-empty で末尾削除 / ×ボタン）、notes textarea（600ms debounce で save、persisted ref で同値書き戻し抑止）。各 mutation 後に `bumpRefresh()` で FlatView/TreeView 再 fetch。未 indexed ファイル（file_id 空）は editor 無効化 + warning。バグ修正で `<ViolationBadges>` に `className` prop（デフォ `ml-auto` 維持、inspector では `className=""` で行頭揃え）+ tag Enter 連打時の二重追加を pendingAdd ref + `setDraft("")` 同期化 + 楽観挿入の dedupe で防止。`mise run check` + `pnpm build` (472 KB / 147 KB gzip) green。`tauri dev` 実機 smoke（FlatView/TreeView/palette からファイルクリック → inspector / タグ追加削除（重複なし） / notes 編集 → 自動保存 / project 切替時の selection リセット）は次セッションへ持ち越し
  - [x] project init UI（feat/project-init-ui）— `core::project::preview_init(target) -> InitPreview { target_path, target_exists, is_existing_project, predicted_file_count, artifacts, gitignore_exists }` を新設（予測ファイル数は scanner と同じ `Scanner` + `IgnoreRules::from_patterns(DEFAULT_PATTERNS)` で walk、init 後の最初の reconcile が拾うのと同じ件数を保証する regression test 込み 6 unit test）。`progest-tauri` に IPC 3 つ: `project_init_preview(path)` (pure read) / `project_init_new(parent, name)` (mkdir → `core::project::initialize` → `Reconciler::full_scan` → `ProjectContext::open` → `recent::record` → AppState swap) / `project_init_existing(path, name?)` (basename フォールバック)。`ProjectError::AlreadyInitialized` は `already_initialized:` プレフィックス付き String エラーに正規化、フロント側で discriminator により「Open existing」へルート。`<InitProjectDialog>` (`App.tsx` global mount、shadcn `Dialog` + `ToggleGroup` で New / Existing 切替、両モードで 150ms debounce の preview API 呼び出し → target path / 予測ファイル数 / artifact 一覧 / `.gitignore` create-or-append ラベル表示、`is_existing_project` または `isAlreadyInitialized(submitError)` 時に primary CTA を「Open existing project」に差替)、Welcome 画面に `New project…` / `Initialize folder…` ボタン、`>command` パレットに `New project…` / `Initialize existing folder…` 追加（`docs/COMMAND_PALETTE.md §1.1` 同期）。`useProject()` を `initNew` / `initExisting` / `openByPath` / `openInitDialog` / `closeInitDialog` で拡張。typecheck + `vp check` + cargo fmt + clippy + `pnpm build` (467 KB / 146 KB gzip) green。`tauri dev` 実機 smoke（New / Existing 両フロー / `>new project` / 既存 `.progest/` 検出時の Open existing 切替 / scan 後の FlatView 反映）は次セッションへ持ち越し
  - [x] M3 #9 ディレクトリインスペクター + `accepts.{read,write}` IPC + lint refresh — `core::accepts::writer`（`inject_accepts` / `remove_accepts`、token 順序保持、7 round-trip test）。`progest-tauri::accepts_commands::{accepts_read, accepts_write}`（own / effective / chain / alias catalog / warnings + tagged-union token wire、mode/alias 検証 + `expand_own_accepts` で書込前に unknown-alias チェック、7 unit test）。`progest-tauri::lint_commands::lint_run`（rules.toml + schema.toml + project.toml [cleanup] を inline でロード、`Index::list_files` から path 集合を作って `lint_paths` → `write_to_index`）。`app/` 側は `<DirectoryInspector>` を新設（own/effective/chain/alias プレビュー、chip 入力で `.ext`/`:alias` を inline parse、inherit checkbox、mode セレクタ、alias 展開ドロワー、保存後に `lintRun` を自動 trigger して footer に scanned/naming/placement/sequence を表示）、`<TreeView>` がディレクトリ選択を支援（クリックで toggle + select、選択行は `bg-accent`）。`App.tsx` を `<ResizablePanelGroup>` (Tree 22 / Flat 40 / Inspector 38、id `progest:main-shell` で永続化) に組み替え。`ProjectContext` に `refreshTick` + `bumpRefresh()` を追加し、FlatView は search effect deps、TreeView は visited path 全体を再 fetch する追加 effect で連動。placement バッジ色は PR #36 の `--badge-placement` semantic token を流用。`mise run check` + `pnpm build` green。`tauri dev` 実機 smoke (Cmd+K / accepts 編集 → 保存 → flat/tree 違反バッジが新しい dirmeta を反映 / `is:misplaced` 動作 / 3-pane 可変分割) は次セッションで実施（feat/m3-directory-inspector）
- **M4 サムネ + 外部連携 + AI + テンプレート**: **進行中**
  - [x] `core::import` — accepts ランキング + import preview + atomic apply + history `Operation::Import` + Conflict 4 種 + `ApplyWarning::ImportFailed` variant（feat/m4-core-import、PR #59）
  - [x] CLI `progest import` — `<files...> [--dest|--move|--dry-run|--format]`、tty 検出で対話/非対話（feat/m4-core-import、PR #59）
  - [x] CLI `-p/--project` グローバルオプション（feat/cli-project-option、PR #60）
