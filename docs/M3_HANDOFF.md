...
最終更新: 2026-04-27（M3 polish ラウンド完了: theme / DSL list+alias / 空クエリ全件 / project 切替 refresh / `>command` パレット / status bar）
...
| 9 | ディレクトリインスペクター + placement バッジ + `accepts.{read,write}` IPC | tree/flat view | accepts 編集フォーム（chip input + inherit checkbox + mode セレクタ）→ `.dirmeta.toml` 書込。flat / tree 上で placement 違反バッジ（naming とは別色）。`is:misplaced` クエリ動作確認も同 PR |
