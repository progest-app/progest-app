...
**M1 Core data layer（完了）**。M0 足場 + `core::fs` / `core::identity` / `core::meta` / `core::index` / `core::reconcile` / `core::watch` / `core::project` / CLI `init`/`scan`/`doctor` / 10k-file incremental scan ベンチ（実測 ~82 ms、5 s gate の 60 倍下回り）が揃い、M1 完了条件を満たした。

**M2 Naming rules engine + accepts（進行中）**。最初のスライスとして `core::meta` の残タスク（pending queue / `.dirmeta.toml` loader）が landed。次は `core::rules`（DSL パーサ → eval → template）、その後 `core::accepts` / `core::history` / `core::rename` → CLI `lint`/`rename`/`undo`/`redo` の順。
...
- [x] `core::meta` — PR #5 で `MetaDocument` TOML schema（forward-compat round-trip）、`MetaStore` trait + `StdMetaStore`、`sidecar_path` が landed。M2 冒頭で残タスクを解消：
  - [x] `.progest/local/pending/` 失敗書込キュー：envelope TOML（target + document_toml + attempts + last_error）を blake3-of-target 名で書き、`StdMetaStore` の load/save/delete が毎回暗黙に `flush_pending` を呼ぶ。明示 API も `flush_pending()` で提供
  - [x] `.dirmeta.toml` loader：`DirmetaDocument`（tags / notes / custom / meta_internal を typed、`[accepts]` を含むその他キーは `extra: toml::Table` で round-trip）、`load_dirmeta` / `save_dirmeta` 関数。`core::accepts` は `document.extra.get("accepts")` で読み込む設計
...
- [x] M1 完了条件ベンチ（10k files < 5s、incremental scan ベース）— `crates/progest-core/benches/scan.rs`、criterion、`cargo bench -p progest-core --bench scan`。実測 ~82 ms（5 s 予算の 1.6%）。初回 bulk `.meta` 生成は予算外で計測除外、incremental path のみを測る
