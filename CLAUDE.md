...
現在のフェーズ: **M2 Naming rules engine + accepts 進行中**。M1 Core data layer 完了、`core::rules`（DSL loader / template / constraint / inheritance / evaluate）・`core::accepts`（`[accepts]` / alias catalog / effective_accepts / placement lint）・`core::naming`（cleanup pipeline / NameCandidate + Hole / `[cleanup]` loader / `FillMode` / `suggest::fill_suggested_names` / CLI `progest clean` preview）・`core::history`（SQLite `.progest/local/history.db` / 5 op kind / pure `invert()` / undo/redo + retention 50）が landed。次は `core::rename`（preview / apply、原子トランザクション、naming の `NameCandidate` を入力に取り history へ append、`progest clean --apply` / `FillMode::Prompt` もここで合流）→ CLI `lint` / `rename` / `undo` / `redo`。進捗詳細は `docs/IMPLEMENTATION_PLAN.md §0`、次モジュールの kickoff 質問テンプレは `docs/M2_HANDOFF.md §5` 参照。
