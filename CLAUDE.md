...
**M1 Core data layer（完了）**。M0 足場 + `core::fs` / `core::identity` / `core::meta`（初スライス）/ `core::index` / `core::reconcile` / `core::watch` / `core::project` / CLI `init`/`scan`/`doctor` / 10k-file incremental scan ベンチ（実測 ~82 ms、5 s gate の 60 倍下回り）が揃い、M1 完了条件を満たした。次は M2 Naming rules engine + accepts。
