...
現在のフェーズ: **M3 検索とビュー 進行中**。M1 Core data layer 完了、M2 完了（`core::rules` / `core::accepts` / `core::naming` / `core::history` / `core::rename` / `core::sequence` / `core::sequence::drift` / `core::lint` 全コア + CLI `init` / `scan` / `doctor` / `clean` / `rename` / `lint` / `undo` / `redo` + history retention 50）。M3 は `core::search` + FTS5 + コマンドパレット + tree/flat view + ディレクトリインスペクター + CLI `search`/`tag` + Tauri IPC 同時実装が範囲。DSL 仕様書 [`docs/SEARCH_DSL.md`](./docs/SEARCH_DSL.md) landed（feat/m3-search-dsl-spec）、次は `core::search` parser/planner。`core::import` + thumbnail + AI + template は **M4**。進捗詳細は `docs/IMPLEMENTATION_PLAN.md §0`・§5、M3 進行中の引き継ぎは `docs/M3_HANDOFF.md`、M4 import kickoff 向け sequence 統合設計メモは `docs/M2_HANDOFF.md §5` 参照。
...
- **フロントで native `<input type="checkbox">` / `<select>` / カスタム rounded button を直書きする**（必ず shadcn primitive を使う。Checkbox / Select / ToggleGroup / Label 等。理由とハマりどころは `docs/LESSONS.md §17`）
...
進捗スナップショットは [`docs/IMPLEMENTATION_PLAN.md §0`](./docs/IMPLEMENTATION_PLAN.md)、M3 進行中の詳細な引き継ぎは [`docs/M3_HANDOFF.md`](./docs/M3_HANDOFF.md)（M2_HANDOFF は M2 完了アーカイブとして残置）。作業一区切りごとに両方を更新する。
...
- [docs/M3_HANDOFF.md](./docs/M3_HANDOFF.md) — **M3 進行中の引き継ぎメモ**（次に触るモジュール順、focus areas、kickoff 質問テンプレ）
- [docs/SEARCH_DSL.md](./docs/SEARCH_DSL.md) — M3 search DSL 仕様書（parser / planner 着手前に必読）
- [docs/M2_HANDOFF.md](./docs/M2_HANDOFF.md) — M2 完了アーカイブ（M4 import kickoff 向け sequence 統合設計メモを §5 に保持）
...
- [docs/REFACTOR_BACKLOG.md](./docs/REFACTOR_BACKLOG.md) — 着手していないリファクタ候補一覧（やったら消す / 増えたら追記）
...
6. **作業が一段落したらドキュメントの更新・整理を必ず行う。** 対象:
   ...
   - `docs/M3_HANDOFF.md` の進行中メモ（該当フェーズのみ）
