...
現在のフェーズ: **M1 Core data layer 進行中**。M0 Skeleton 完了、`progest-core::fs` と `progest-core::identity` が landed（PR #3 merged、PR #4 in review）。次は `core::meta`。
...
**M1 Core data layer（進行中）**。M0 足場は出来上がり、`core::fs` が最初のモジュールとして landed（PR #3）。
...
- [ ] `core::meta`（TOML I/O、原子書込、pending queue）
