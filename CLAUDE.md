...
現在のフェーズ: **M2 Naming rules engine + accepts 完了**。M1 Core data layer 完了、M2 は `core::rules` / `core::accepts` / `core::naming` / `core::history` / `core::rename` / `core::sequence` / `core::sequence::drift` / `core::lint` の全コア + CLI `init` / `scan` / `doctor` / `clean` (preview + --apply + sequence-aware) / `rename` (preview + apply + --sequence-stem) / `lint` (grouped JSON/text + --explain + exit code DSL §8.2) / `undo` / `redo` (group-aware / --entry) + history retention 50 が landed。次は **M3 検索とビュー**（`core::search` + FTS5 + コマンドパレット + tree/flat view + ディレクトリインスペクター + CLI `search`/`tag`）。`core::import` + thumbnail + AI + template は **M4**。進捗詳細は `docs/IMPLEMENTATION_PLAN.md §0`・§5、M4 import kickoff 向け sequence 統合設計メモは `docs/M2_HANDOFF.md §5` 参照。
